...
- **Time-Based Filtering**: Narrow down results to jobs posted within the last hour, day, week, month, or year.
...
- **Persistent Inputs**: Your last search criteria (keywords, location, work type, and date posted) are saved in local storage, so you can pick up where you left off.
...
4.  **Filter by Date Posted (Optional)**: Choose a time frame like `Past 24h` or `Past Week` to see the most recent listings.
5.  **Search**:
...
6.  **Clear**: Click the **Clear Fields** button to reset all inputs and remove the saved criteria from your browser.
